...
namespace SVS {
    ...
    QSize TitleListItemDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const {
        ...
        auto icon = index.data(SVS::DecorationRole).value<QIcon>();
        auto location = index.data(SVS::SubtitleRole).toString();
        auto date = index.data(SVS::DescriptionRole).toString();

        auto iconSize = index.data(SVS::IconSizeRole).toSize();
        if (iconSize.isEmpty()) {
    ...
    void TitleListItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
        ...
        // Fetch data
        QString filename = index.data(SVS::DisplayRole).toString();
        QString location = index.data(SVS::SubtitleRole).toString();
        QString date = index.data(SVS::DescriptionRole).toString();

        auto icon = index.data(SVS::DecorationRole).value<QIcon>();
        QSize iconSize = index.data(SVS::IconSizeRole).toSize();
        if (iconSize.isEmpty()) {
            iconSize = d->m_defaultIconSize;
        }
        if (icon.isNull()) {
        ...
        Qt::AlignmentFlag dateVAlign = static_cast<Qt::AlignmentFlag>(index.data(SVS::AlignmentRole).toInt());
        ...
        if (index.data(SVS::SeparatorRole).toBool()) {
