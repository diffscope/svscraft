...
namespace SVS {
    ...
    void SeekBarPrivate::calculateParams() {
        ...
        activeStartPos = int(actualLength * (trackActiveStartValue - minimum) / (maximum - minimum)) + padding;
    ...
    SeekBar::SeekBar(QWidget *parent) : SeekBar(parent, *new SeekBarPrivate) {
        ...
        connect(d->thumbHoverAnimation, &QVariantAnimation::valueChanged, d, &SeekBarPrivate::setThumbBorderRatio);
        d->calculateParams();
    }
    ...
    void SeekBar::paintEvent(QPaintEvent *event) {
        ...
        d->valuePos = int(d->actualLength * (d->sliderValue - d->minimum) / (d->maximum - d->minimum)) + d->padding;
    ...
    void SeekBar::mouseMoveEvent(QMouseEvent *event) {
        Q_D(SeekBar);
        auto pos = event->pos();
        auto posValue = (pos.x() - d->padding) * (d->maximum - d->minimum) / d->actualLength + d->minimum;
